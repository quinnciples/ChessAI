            ...
            for PIECE_COLOR in [Piece.WHITE, Piece.BLACK]:
                for PIECE_MOVED in (0, 1):
                    for BOARD_INDEX in range(64):  # 64
                        ...
                        elif PIECE_TYPE in [Piece.PAWN] and PIECE_COLOR == Piece.WHITE:
                            new_square = BOARD_INDEX - 8
                            if new_square >= 0:
                                up_moves.append(new_square)
                            if PIECE_MOVED == 0:
                                new_square = BOARD_INDEX - 16
                                if new_square >= 0:
                                    up_moves.append(new_square)
                        ...
                        elif PIECE_TYPE in [Piece.PAWN] and PIECE_COLOR == Piece.BLACK:
                            new_square = BOARD_INDEX + 8
                            if new_square < 64:
                                down_moves.append(new_square)
                            if PIECE_MOVED == 0:
                        ...
                        # self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)]['UP'] = up_moves
                        # self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)]['DOWN'] = down_moves
                        # self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)]['LEFT'] = left_moves
                        # self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)]['RIGHT'] = right_moves
                        # self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)]['UP_LEFT'] = up_left_moves
                        # self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)]['UP_RIGHT'] = up_right_moves
                        # self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)]['DOWN_RIGHT'] = down_right_moves
                        # self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)]['DOWN_LEFT'] = down_left_moves
                        # self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)]['KNIGHT'] = knight_moves
                        if up_moves:
                            self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)].append(up_moves)
                        if down_moves:
                            self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)].append(down_moves)
                        if left_moves:
                            self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)].append(left_moves)
                        if right_moves:
                            self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)].append(right_moves)

                        if up_right_moves:
                            self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)].append(up_right_moves)
                        if down_right_moves:
                            self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)].append(down_right_moves)
                        if down_left_moves:
                            self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)].append(down_left_moves)
                        if up_left_moves:
                            self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)].append(up_left_moves)

                        if knight_moves:
                            for km in knight_moves:
                                self.move_list[(PIECE_COLOR | PIECE_TYPE | PIECE_MOVED, BOARD_INDEX)].append([km])
